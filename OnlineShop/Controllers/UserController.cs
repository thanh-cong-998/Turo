...
namespace OnlineShop.Controllers
{
    public class UserController : Controller
    {
        ...
        public JsonResult LoadPrecinctID(int districtID)
        {
            var xmlDoc = XDocument.Load(Server.MapPath(@"~/Assets/Client/data/Provinces_Data.xml"));

            var xElementss = xmlDoc.Element("Root").Elements("Item").Elements("Item")
            ...
            foreach (var item in xElementss.Elements("Item").Where(x => x.Attribute("type").Value == "precinct"))
            {
                precinct = new PrecinctModel();
                precinct.ID = int.Parse(item.Attribute("id").Value);
                precinct.Name = item.Attribute("value").Value;
                precinct.DistrictID = int.Parse(xElementss.Attribute("id").Value);
                list.Add(precinct);
